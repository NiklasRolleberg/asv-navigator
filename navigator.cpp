...
void Navigator::goToCoordinates(Task* task)
{
    ...
    double startLat = data->getLat();
    double startLon = data->getLon();

    data->setBoatWaypoint_real(startLat,startLon,lat, lon,1.6);
    ...
    while(d > tol)
    {
      ...
      if(!data->hasCorrectPath(startLat,startLon,lat,lon,2)) {
